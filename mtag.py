...
def _read_SNPlist(file_path, SNP_index):
    ...
    if SNP_index not in snplist.columns:
        raise ValueError(
            "SNPlist read from {} does not include --snp_name {} in its columns.".format(
                file_path, SNP_index
            )
        )
...
def _read_matrix(file_path):
    ...
    if ext == ".txt":
        ...
        raise ValueError(
            "{} is not one of the acceptable file paths for reading in matrix-valued objects.".format(
                ext
            )
        )
...
def _perform_munge(args, GWAS_df, GWAS_dat_gen, p):
    ...
    if args.use_beta_se:
        argnames = Namespace(
            ...
            N_con=None,
            out=out,
            maf_min=args.maf_min_list[p],
            info_min=args.info_min_list[p],
            daner=False,
            no_alleles=False,
            merge_alleles=merge_alleles,
            n_min=args.n_min_list[p],
    ...
    munged_results = munge_sumstats.munge_sumstats(
        argnames, write_out=False, new_log=False
    )
    GWAS_df = GWAS_df.merge(
        munged_results,
        how="inner",
        left_on=args.snp_name,
        right_on="SNP",
        suffixes=("", "_ss"),
    )
    ...
    logging.info(borderline)
    logging.info(
        "Munging of Trait {} complete. SNPs remaining:\t {}".format(p + 1, len(GWAS_df))
    )
    logging.info(borderline + "\n")
...
def load_and_merge_data(args):
    ...
    if args.n_value is not None:
        ...
        assert P == len(
            args.n_list
        ), "Mismatch of length of --n_value and number of summary statistics."
    ...
    for p, GWAS_input in enumerate(GWAS_input_files):
        ...
        logging.info(
            "Read in Trait {} summary statistics ({} SNPs) from {} ...".format(
        ...
        if z_checker < 1.02 and not args.force:
            raise ValueError(
                "The mean chi2 statistic of trait {} is less than 1.02, which may lead to unstable estimates. To perform MTAG on your results anyways, include the --force option, though the estimates should be interpreted cautiously.".format(
                    p + 1
                )
            )
        ...
        # conform column names internally
            ...
            if z_checker < 1.02:
                logging.info(
                    "Warning: The mean chi2 statistic of trait {} is less 1.02 - MTAG estimates may be unstable.".format(
                        p + 1
                    )
                )
            GWAS_d[p].rename(
                columns=munge_sumstats.set_default_cnames(args), inplace=True
            )
        ...
        if (
            args.info_min_list[p] is not None
            and "INFO{}".format(p) not in GWAS_d[p].columns
        ):
            raise IOError(
                "--info_min is specified but info column is not present in sumstats {}".format(
                    p + 1
                )
            )
        if (
            args.maf_min_list[p] is not None
            and "FRQ{}".format(p) not in GWAS_d[p].columns
        ):
            raise IOError(
                "--maf_min is specified but maf column is not present in sumstats {}".format(
                    p + 1
                )
            )
        if args.n_min_list[p] is not None and "N{}".format(p) not in GWAS_d[p].columns:
            raise IOError(
                "--n_min is specified but n column is not present in sumstats {}".format(
                    p + 1
                )
            )
        ...
        if M0 - len(GWAS_d[p]) > 0:
            logging.info(
                'Trait {}: Dropped {} SNPs for missing values in the "snp_name" column'.format(
                    p + 1, M0 - len(GWAS_d[p])
                )
            )
        ...
        if M0 - len(GWAS_d[p]) > 0:
            logging.info(
                'Trait {}: Dropped {} SNPs for duplicate values in the "snp_name" column'.format(
                    p + 1, M0 - len(GWAS_d[p])
                )
            )
    ...
    for p in range(P):
        if p == 0:
            ...
            if args.meta_format:
                ...
                GWAS_all = GWAS_all.merge(
                    GWAS_d[p], how="outer", on="SNP", indicator=True
                )
                GWAS_all.loc[
                    np.logical_or(
                        GWAS_all._merge == "both", GWAS_all._merge == "right_only"
                    ),
                    "Trait{}".format(p),
                ] = 1
                GWAS_all.loc[GWAS_all._merge == "left_only", "Trait{}".format(p)] = 0
                GWAS_all.loc[
                    GWAS_all._merge == "right_only", "Trait{}".format(p - 1)
                ] = 0
            ...
            M_0 = len(GWAS_int)
            snps_to_flip = np.logical_and(
                GWAS_int["A1" + str(0)] == GWAS_int["A2" + str(p)],
                GWAS_int["A2" + str(0)] == GWAS_int["A1" + str(p)],
            )
            GWAS_int["flip_snps" + str(p)] = snps_to_flip

            snps_to_keep = np.logical_or(
                np.logical_and(
                    GWAS_int["A1" + str(0)] == GWAS_int["A1" + str(p)],
                    GWAS_int["A2" + str(0)] == GWAS_int["A2" + str(p)],
                ),
                snps_to_flip,
            )
            ...
            if len(GWAS_int) < M_0:
                logging.info(
                    "Dropped {} SNPs due to inconsistent allele pairs from phenotype {}. {} SNPs remain.".format(
                        M_0 - len(GWAS_int), p + 1, len(GWAS_int)
                    )
                )
            ...
            if np.sum(snps_to_flip) > 0:
                zz = "Z"
                freq_name = "FRQ"
                GWAS_int.loc[snps_to_flip, zz + str(p)] = (
                    -1 * GWAS_int.loc[snps_to_flip, zz + str(p)]
                )
                GWAS_int.loc[snps_to_flip, freq_name + str(p)] = (
                    1.0 - GWAS_int.loc[snps_to_flip, freq_name + str(p)]
                )
                store_allele = GWAS_int.loc[snps_to_flip, "A1" + str(p)]
                GWAS_int.loc[snps_to_flip, "A1" + str(p)] = GWAS_int.loc[
                    snps_to_flip, "A2" + str(p)
                ]
                GWAS_int.loc[snps_to_flip, "A2" + str(p)] = store_allele
                logging.info(
                    "Flipped the signs of of {} SNPs to make them consistent with the effect allele orderings of the first trait.".format(
                        np.sum(snps_to_flip)
                    )
                )

        STRAND_AMBIGUOUS_SET = [
            x
            for x in allele_info.STRAND_AMBIGUOUS.keys()
            if allele_info.STRAND_AMBIGUOUS[x]
        ]

        GWAS_int["strand_ambig"] = (
            GWAS_int["A1" + str(0)].str.upper() + GWAS_int["A2" + str(0)].str.upper()
        ).isin(STRAND_AMBIGUOUS_SET)

        if not args.incld_ambig_snps:
            M_0 = len(GWAS_int)
            GWAS_int = GWAS_int[np.logical_not(GWAS_int["strand_ambig"])]
            logging.info(
                "Dropped {} SNPs due to strand ambiguity, {} SNPs remain in intersection after merging trait{}".format(
                    M_0 - len(GWAS_int), len(GWAS_int), p + 1
                )
            )
        else:
            logging.info(
                "{} strand ambiguous SNPs in Trait {} are included.".format(
                    np.sum(GWAS_int["strand_ambig"]), p + 1
                )
            )

    logging.info(
        "... Merge of GWAS summary statistics complete. Number of SNPs:\t {}".format(
            len(GWAS_int)
        )
    )
    ...
    if args.include is not None:
        for j, include_file in enumerate(args.include.split(",")):
            if j == 0:
                ...
                snps_include = snps_include.merge(
                    _read_SNPlist(include_file, "SNP"), how="outer", on="SNP"
                )
        GWAS_all = GWAS_all.merge(
            snps_include,
            how="left",
            on="SNP",
            indicator="included_merge",
            suffixes=("", "_incl"),
        )
        GWAS_all = GWAS_all.loc[GWAS_all["included_merge"] == "both"]
        GWAS_all = GWAS_all.loc[:, GWAS_orig_cols]
        logging.info(
            "(--include) Number of SNPs remaining after restricting to SNPs in the union of  {include_path}: \t {M} remain".format(
                include_path=args.include, M=len(GWAS_all)
            )
        )
    ...
    if args.exclude is not None:
        for exclude_file in args.exclude.split(","):
            ...
            GWAS_all = GWAS_all.merge(
                ...
                how="left",
                on="SNP",
                indicator="excluded_merge",
                suffixes=("", "_incl"),
            )
            GWAS_all = GWAS_all.loc[GWAS_all["excluded_merge"] == "left_only"]
            GWAS_all = GWAS_all.loc[:, GWAS_orig_cols]
            logging.info(
                "(-exclude) Number of SNPs remaining after excluding to SNPs in {exclude_path}: \t {M} remain".format(
                    exclude_path=exclude_file, M=len(GWAS_all)
                )
            )
...
def estimate_sigma(data_df, args):
    ...
    args_ldsc_rg = Namespace(
        ...
        rg=rg_files,
        ref_ld=None,
        ref_ld_chr=args.ld_ref_panel,
        w_ld=None,
        w_ld_chr=args.ld_ref_panel,
        overlap_annot=False,
        no_intercept=False,
        intercept_h2=None,
        ...
        frqfile_chr=None,
        print_all_cts=False,
        sumstats_frames=[gwas_ss_df[i] for i in range(args.P)],
        rg_mat=rg_mat,
    )
    ...
    if args.no_overlap:
        ...
        for t in range(args.P):
            ...
            sigma_hat[t, t] = ldsc_matrix_formatter(rg_results_t, ".gencov.intercept")[
                0
            ]
    else:
...
def extract_gwas_sumstats(DATA, args, t0):
    ...
    if args.homogNs_frac is not None or args.homogNs_dist is not None:
        ...
        if args.homogNs_frac is not None:
            logging.info(
                "--homogNs_frac {} is on, filtering SNPs ...".format(args.homogNs_frac)
            )
            assert args.homogNs_frac >= 0.0
            ...
            for p in t0:
                N_nearMode[:, p] = (
                    np.abs((Ns[:, p] - N_modes[p])) / N_modes[p] <= homogNs_frac_list[p]
                )
        elif args.homogNs_dist is not None:
            logging.info(
                "--homogNs_dist {} is on, filtering SNPs ...".format(args.homogNs_dist)
            )
            ...
            raise ValueError(
                "Cannot specify both --homogNs_frac and --homogNs_dist at the same time."
            )
        ...
        for p in range(Ns.shape[1]):
            mode_restrictions += (
                "Phenotype {}: \t {} SNPs pass modal sample size filter \n".format(
                    p + 1, np.sum(N_nearMode[:, p])
                )
            )

        mode_restrictions += "Intersection of SNPs that pass modal sample size filter for all traits:\t {}".format(
            np.sum(np.all(N_nearMode, axis=1))
        )
    ...
    if args.n_max is not None:
        n_max_restrictions = (
            "--n_max used, removing SNPs with sample size greater than  {}".format(
                args.n_max
            )
        )
        ...
        for p in range(Ns.shape[1]):
            n_max_restrictions += (
                "Phenotype {}: \t {} SNPs pass modal sample size filter".format(
                    p + 1, np.sum(N_passMax[:, p])
                )
            )
        n_max_restrictions += "Intersection of SNPs that pass maximum sample size filter for all traits:\t {}".format(
            np.sum(np.all(N_passMax, axis=1))
        )
...
def jointEffect_probability(Z_score, omega_hat, sigma_hat, N_mats, S=None):
    ...
    Zs_rep = np.einsum(
        "mp,s->msp", Z_score, np.ones(n_S)
    )  # functionally equivalent to repmat
    ...
    logSqrtDetSigmas = np.sum(
        np.log(np.diagonal(Rs, axis1=2, axis2=3)), axis=2
    ).reshape(M, n_S)
...
def numerical_omega(args, Zs, N_mats, sigma_LD, omega_start):
    ...
    if args.perfect_gencov:
        return (
            np.sqrt(np.outer(np.exp(opt_results.x), np.exp(opt_results.x))),
            opt_results,
        )
    else:
...
def flatten_out_omega(omega_est):
    ...
    x_chol_trf[np.diag_indices(P_c)] = np.log(
        np.diag(x_chol)
    )  # replace with log transformation on the diagonal
...
def rebuild_omega(chol_elems, s=None):
    ...
    """
    if s is None:
        P = int((-1 + np.sqrt(1.0 + 8.0 * len(chol_elems))) / 2.0)
    ...
    cholL[np.diag_indices(P_c)] = np.exp(
        np.diag(cholL)
    )  # exponentiate the diagnoal so cholL unique
    for i in range(P_c):
...
def estimate_omega(args, Zs, Ns, sigma_LD, omega_in=None):
    ...
    if args.numerical_omega:
        ...
        numerical_msg += "\nSuccessful termination? {}".format(
            "Yes" if opt_results.success else "No"
        )
...
def mtag_analysis(Zs, Ns, omega_hat, sigma_LD):
    ...
    Sigma_N = np.einsum(
        "mpq,mqr->mpr", np.einsum("mpq,qr->mpr", W_N_inv, sigma_LD), W_N_inv
    )
    ...
    for p in range(P):
        ...
        mtag_factor[:, p] = np.einsum(
            "mp,m->m", np.einsum("q,mqp->mp", yy, inv_xx), 1 / beta_denom
        )
        mtag_var_p = 1.0 / beta_denom

        mtag_betas[:, p] = (
            np.einsum("mp,mp->m", np.einsum("q,mqp->mp", yy, inv_xx), W_inv_Z)
            / beta_denom
        )
...
def save_mtag_results(
    args, results_template, Zs, Ns, Fs, mtag_betas, mtag_se, mtag_factor
):
    """
    ...
    """
    p_values = lambda z: 2 * (scipy.stats.norm.cdf(-1.0 * np.abs(z)))
    ...
    if args.equal_h2 and args.perfect_gencov:
        logging.info(
            "With meta-analysis mode, MTAG produces a single set of sumstats, where betas are unstandardized using 2p(1-p) where p is the average allele frequencies across traits."
        )
        ...
        for p in range(P):
            ...
            if args.std_betas:
                ...
                weights = np.sqrt(2 * Fs[:, p] * (1.0 - Fs[:, p]))
...
def write_summary(args, Zs, Ns, Fs, mtag_betas, mtag_se, mtag_factor):
    ...
    input_phenotypes = [
        "..." + f[-16:] if len(f) > 20 else f for f in args.sumstats.split(",")
    ]
    ...
    for p in range(P):
        ...
        if args.meta_format:
            comb_df_extract = [
                Ns[y][x] for y in Ns.keys() for x in Ns[y].keys() if x == p
            ]
        ...
        summary_df.loc[p + 1, "GWAS mean chi^2"] = (
            np.mean(np.square(Zs[:, p])) / args.sigma_hat[p, p]
        )
        Z_mtag = mtag_betas[:, p] / mtag_se[:, p]
        summary_df.loc[p + 1, "MTAG mean chi^2"] = np.mean(np.square(Z_mtag))
        summary_df.loc[p + 1, "GWAS equiv. (max) N"] = int(
            summary_df.loc[p + 1, "N (max)"]
            * (summary_df.loc[p + 1, "MTAG mean chi^2"] - 1)
            / (summary_df.loc[p + 1, "GWAS mean chi^2"] - 1)
        )
...
def save_mtag_results_U(args, comb_df):
    ...
    for p in range(args.P):
        logging.info("Writing Phenotype {} to file...".format(p + 1))
        comb_df_extract = [
            comb_df[y][x] for y in comb_df.keys() for x in comb_df[y].keys() if x == p
        ]
        out_df = pd.concat(comb_df_extract, axis=0)
        M_0 = out_df.shape[0]
...
def MTAG_var_Z_jt_c(t, Omega, Omega_c, sigma_LD, Ns):
    ...
    numer = np.einsum(
        "mp,mp->m", num_L, np.einsum("mpq,mq->mp", Omega_c + Sigma_j, num_R)
    )

    denom = np.einsum(
        "p,mp->m", gamma_k / tau_k_2, np.einsum("mpq,q->mp", inv_xx, gamma_k / tau_k_2)
    )
...
def compute_fdr(prob, t, omega, sigma, S, Ns, N_counts, p_threshold):
    ...
    z_threshold = scipy.stats.norm.isf(
        p_threshold / 2.0
    )  # magnitude of z-score needed for statistical significance
    n_S, T = S.shape
    ...
    for k in range(len(S)):
        sd = np.sqrt(
            MTAG_var_Z_jt_c(t, omega, Omega_s[k, :, :], sigma, Ns)
        )  # ZZZ generalize to take in Omega_s rather than one state at a time .
        Prob_signif_cond_t[k] = np.sum(
            2 * scipy.stats.norm.sf(z_threshold, loc=0, scale=sd) * N_counts
        ) / float(
            np.sum(N_counts)
        )  # produces m FDR estimates: take average by weighting each unique sample size row with the counts of SNPs with that sample size (weighted average, denominator will be equal to M)
...
def neglogL_single_SS(x, beta, se, transformed=True):
    ...
    return -1.0 * np.sum(
        np.log((1.0 - prob_null) * causal_pdf + prob_null * noncausal_pdf)
    )
...
def _optim_ss(f_args):
    ...
    # beta_t, se_t = f_args
    optim_results = scipy.optimize.minimize(
        neglogL_single_SS,
        x_0,
        args=(beta_t, se_t, True),
        method="Nelder-Mead",
        options=solver_opts,
        callback=None,
    )
...
def ss_estimation(
    ...
    arg_list_ss = [
        (betas[:, t], se[:, t], starting_params, solver_opts) for t in range(T)
    ]
    ss_results = joblib.Parallel(
        n_jobs=args.cores, backend="multiprocessing", verbose=0, batch_size=1
...
def fdr(args, Ns_f, Zs):
    ...
    if not args.grid_file:
        if args.intervals <= 0:
            raise ValueError(
                "spacing of grid points for the max FDR calculation must be a positive integer"
            )
    ...
    if args.grid_file is not None:
        ...
        prob_grid = prob_grid[
            (np.sum(prob_grid, axis=1) > 1.0) & np.sum(prob_grid, axis=1) < 0
        ]
    else:
    ...
    prob_grid = [
        x
        for x in prob_grid
        if some_causal_for_allT(x, S)
        and is_pos_semidef(scale_omega(args.omega_hat, x, S))
    ]

    if args.fit_ss:
        gwas_se = 1.0 / np.sqrt(Ns)
        gwas_betas = gwas_se * Zs

        ss_params_list = ss_estimation(args, gwas_betas, gwas_se)
        pi_causal_ss = np.array([1.0 - x[0] for x in ss_params_list])
        logging.info(
        ...
        prob_grid = [
            p
            for p in prob_grid
            if np.all(np.abs(causal_prob(p, S) - pi_causal_ss) < (1.0 / args.intervals))
        ]
        logging.info(
            "{} probabilities remain after restricting to the grid points with causal probabilities less than one unit (i.e. 1/intervals) from the Spike-Slab fitted causal probabilities.".format(
                len(prob_grid)
            )
        )
    ...
    arg_list = [
        (probs, args.omega_hat, args.sigma_hat, S, N_vals, N_weights, args.p_sig, g, t)
        for t in range(T)
        for g, probs in enumerate(prob_grid)
    ]
    NN = len(arg_list)
    K = 10
    start_fdr = time.time()
    for k in range(K):
        ...
        grid_results = joblib.Parallel(
            n_jobs=args.cores, backend="multiprocessing", verbose=0, batch_size="auto"
        )(joblib.delayed(_FDR_par)(f_args) for f_args in sublist)
        logging.info(
            "Grid search: {} percent finished for . Time: \t{:.3f} min".format(
                (k + 1) * 100.0 / K, (time.time() - start_fdr) / 60.0
            )
        )
    ...
    np.savetxt(args.out + "_fdr_mat.txt", FDR, delimiter="\t")
    logging.info(
        "Saved calculations of fdr over grid points in {}".format(
            args.out + "_fdr_mat.txt"
        )
    )
    ...
    if args.fit_ss:
        ...
        for t in range(T):
            logging.info(
                "FDR of Trait {}: {} at probs = {}".format(
                    t + 1, max_FDR[t], prob_grid[ind_max[t]]
                )
            )
    else:
        ...
        for t in range(T):
            logging.info(
                "Max FDR of Trait {}: {} at probs = {}".format(
                    t + 1, max_FDR[t], prob_grid[ind_max[t]]
                )
            )
...
def mtag(args):
    ...
    if args.equal_h2 and not args.perfect_gencov:
        raise ValueError(
            "--equal_h2 option used without --perfect_gencov. To use --equal_h2, --perfect_gencov must be also be included."
        )
    ...
    try:
        ...
        logging.info(
            "Invalid path used for --out: must have at least one / (use ./[tag] for current directory) and must not end in /"
        )
        raise
    ...
    if not os.path.isdir(args.outdir):
        if args.make_full_path or args.outdir[0] != "/":
            logging.info(
                "Output folder provided does not exist, creating the directory"
            )
            safely_create_folder(args.outdir)
        else:
            raise ValueError(
                "Could not find output directory:\n {} \n at the specified absolute path. To create this directory, use the --make_full_path option.".format(
                    args.outdir
                )
            )
    ...
    Zs, Ns, Fs, res_temp, DATA, N_raw = extract_gwas_sumstats(
        DATA, args, list(np.arange(args.P))
    )

    logging.info(
        "Using {} SNPs to estimate Omega ({} SNPs excluded due to strand ambiguity)".format(
            len(Zs) - np.sum(DATA["strand_ambig"]), np.sum(DATA["strand_ambig"])
        )
    )
    ...
    assert (
        args.omega_hat.shape[0]
        == args.omega_hat.shape[1]
        == Zs.shape[1]
        == args.sigma_hat.shape[0]
        == args.sigma_hat.shape[1]
    )
    ...
    if args.meta_format:
        ...
        sub_list = [
            x for x in DATA_U.groupby([x for x in DATA_U.columns if "Trait" in x])
        ]
        ...
        for s in range(len(sub_dict)):
            ...
            tl = np.arange(args.P)[
                np.in1d(list(sub_dict.keys())[s], 1)
            ]  # indexes of true trait

            # extract columns that are of p trait
            GWAS_sub = dict()
            for i in tl:
                GWAS_sub[i] = sub_dict[sub_list[s][0]].filter(
                    regex="^SNP|.+{}$".format(i)
                )

            # inner merge sumstats for each trait
            snp_df = reduce(
                lambda L, R: pd.merge(L, R, how="inner", on="SNP"),
                list(GWAS_sub.values()),
            )
            combo_dict[sub_list[s][0]] = snp_df
            logging.info(
                "There are {M} SNPs present in Traits {tl}".format(
                    M=snp_df.shape[0], tl=np.array_str(tl)
                )
            )
            ...
            omega_sub, sigma_sub = (
                args.omega_hat[tl[:, None], tl[None, :]],
                args.sigma_hat[tl[:, None], tl[None, :]],
            )
            ...
            comb_df[sub_list[s][0]] = (
                dict()
            )  # combo's keys are the index of true traits
            ...
            for p, t in enumerate(tl):
                ...
                comb_df[sub_list[s][0]][t]["mtag_pval"] = p_values(
                    comb_df[sub_list[s][0]][t]["mtag_z"]
                )
    ...
    if args.meta_format:
        ...
        save_mtag_results(
            args, res_temp, Zs, N_raw, Fs, mtag_betas, mtag_se, mtag_factor
        )
    ...
    logging.info(
        "MTAG complete. Time elapsed: {}".format(sec_to_str(time.time() - start_time))
    )
...
in_opts = parser.add_argument_group(
    title="Input Files",
    description="Input files to be used by MTAG. The --sumstats option is required, while using the other two options take priority of their corresponding estimation routines, if used.",
)
...
in_opts.add_argument(
    ...
    default=None,
    action="store",
    help="If specified, will read in the residual covariance matrix saved in the file path below and skip the estimation routine. The rows and columns of the matrix must correspond to the order of the GWAS input files specified. FIles can either be in .txt  or .npy format. Use with caution as the genetic covariance matrix specified will be weakly nonoptimal. File must either be in whitespace-delimited .txt  or .npy",
)

out_opts = parser.add_argument_group(
    title="Output formatting",
    description="Set the output directory and common name of prefix files.",
)
out_opts.add_argument(
    "--out",
    metavar="DIR/PREFIX",
    default="./mtag_results",
    type=str,
    help="Specify the directory and name prefix to output MTAG results. All mtag results will be prefixed with the corresponding tag. Default is ./mtag_results",
)
out_opts.add_argument(
    "--make_full_path",
    default=False,
    action="store_true",
    help="option to make output path specified in -out if it does not exist.",
)
out_opts.add_argument(
    "--meta_format",
    default=False,
    action="store_true",
    help="In addition to the typical results file that are restricted to the intersection of SNPs across files, this creates a file of the union of SNPs, with applications of the MTAG estimator restricted to the set of traits for which that SNP is available.",
)

input_formatting = parser.add_argument_group(
    title="Column names of input files",
    description="These options manually pass the names of the relevant summary statistics columns used by MTAG. It is recommended to pass these names because only narrow searches for these columns are performed in the default cases. Moreover, it is necessary that these input files be readable by ldsc's munge_sumstats command.",
)
input_formatting.add_argument(
    ...
    action="store",
    type=str,
    help='Name of the single column that provides the unique identifier for SNPs in the GWAS summary statistics across all GWAS results. Default is "snpid". This the index that will be used to merge the GWAS summary statistics. Any SNP lists passed to ---include or --exclude should also contain the same name.',
)
input_formatting.add_argument(
    "--z_name",
    default="z",
    help="The common name of the column of Z scores across all input files. Default is the lowercase letter z.",
)
input_formatting.add_argument(
    "--beta_name",
    default="beta",
    help="The common name of the column of beta coefficients (effect sizes) across all input files. Must be specified with se.",
)
input_formatting.add_argument(
    "--se_name",
    default="se",
    help="The common name of the column of standard errors of the betas across all input files. Default is lowercase se. Must be specified with --beta_name.",
)
input_formatting.add_argument(
    "--n_name",
    default="n",
    help="the common name of the column of sample sizes in the GWAS summary statistics files. Default is the lowercase letter  n.",
)
input_formatting.add_argument(
    "--n_value",
    default=None,
    metavar="N1, N2,...",
    type=str,
    help="Comma separated sample size values for each GWAS summary statistics files. This option is useful for GWAS input that does not include an N column, e.g. BOLT-LMM.",
)
input_formatting.add_argument(
    "--eaf_name",
    default="freq",
    help='The common name of the column of effect allele frequencies in the GWAS input files. The default is "freq".',
)
input_formatting.add_argument(
    "--no_chr_data",
    default=False,
    action="store_true",
    help="If used, will not use information related to the chromosome and base pair position columns. Use only it chromosome/base pair positional data is missing, but are certain that the snpids correctly identify the SNPs across traits.",
)
input_formatting.add_argument(
    "--chr_name",
    default="chr",
    type=str,
    help='Name of the column containing the chromosome of each SNP in the GWAS input. Default is "chr".',
)
input_formatting.add_argument(
    "--bpos_name",
    default="bpos",
    type=str,
    help='Name of the column containing the base pair of each SNP in the GWAS input. Default is "bpos".',
)
input_formatting.add_argument(
    "--a1_name",
    default="a1",
    type=str,
    help='Name of the column containing the effect allele of each SNP in the GWAS input. Default is "a1".',
)
input_formatting.add_argument(
    "--a2_name",
    default="a2",
    type=str,
    help='Name of the column containing the non-effect allele of each SNP in the GWAS input. Default is "a2".',
)
input_formatting.add_argument(
    "--p_name",
    default="p",
...
filter_opts = parser.add_argument_group(
    ...
    description="The input summary statistics files can be filtered using the options below. Note that there is some default filtering according to sample size and allele frequency, following the recommendations we make in the corresponding paper. All of these column-based options allow a list of values to be passed of the same length as the number of traits ",
)
filter_opts.add_argument(
    "--include",
    default=None,
    metavar="SNPLIST1,SNPLIST2,..",
    type=str,
    help="Restricts MTAG analysis to the union of snps in the list of  snplists provided. The header line must match the SNP index that will be used to merge the GWAS input files.",
)
filter_opts.add_argument(
    "--exclude",
    "--excludeSNPs",
    ...
    metavar="SNPLIST1,SNPLIST2,..",
    type=str,
    help="Similar to the --include option, except that the union of SNPs found in the specified files will be excluded from MTAG. Both -exclude and -include may be simultaneously specified, but -exclude will take precedent (i.e., SNPs found in both the -include and -exclude SNP lists will be excluded).",
)
filter_opts.add_argument(
    "--only_chr",
    metavar="CHR_A,CHR_B,..",
...
filter_opts.add_argument(
    ...
    action="store",
    metavar="FRAC",
    help="Restricts to SNPs within FRAC of the mode of sample sizes for the SNPs as given by (N-Mode)/Mode < FRAC. This filter is not applied by default.",
)
filter_opts.add_argument(
    "--homogNs_dist",
    default=None,
    ...
    action="store",
    metavar="D",
    help="Restricts to SNPs within DIST (in sample size) of the mode of sample sizes for the SNPs. This filter is not applied by default.",
)
filter_opts.add_argument(
    "--maf_min",
    default="0.01",
    type=str,
    action="store",
    help="set the threshold below SNPs with low minor allele frequencies will be dropped. Default is 0.01. Set to 0 to skip MAF filtering.",
)
filter_opts.add_argument(
    "--n_min",
    default=None,
    type=str,
    action="store",
    help="set the minimum threshold for SNP sample size in input data. Default is 2/3*(90th percentile). Any SNP that does not pass this threshold for all of the GWAS input statistics will not be included in MTAG.",
)
filter_opts.add_argument(
    "--n_max",
    default=None,
    type=str,
    action="store",
    help="set the maximum threshold for SNP sample size in input data. Not used by default. Any SNP that does not pass this threshold for any of the GWAS input statistics will not be included in MTAG.",
)
filter_opts.add_argument(
    "--info_min",
    default=None,
    type=str,
    help="Minimim info score for filtering SNPs for MTAG.",
)
filter_opts.add_argument(
    "--incld_ambig_snps",
    default=False,
    action="store_true",
    help="Include strand ambiguous SNPs when performing MTAG. by default, they are not used to estimate Omega or Sigma.",
)
filter_opts.add_argument(
    "--no_allele_flipping",
    default=False,
    action="store_true",
    help="Prevents flipping the effect sizes of summary statistics when the effect and non-effect alleles are reversed (reletive the first summary statistics file.",
)

special_cases = parser.add_argument_group(
    title="Special Cases",
    description="These options deal with notable special cases of MTAG that yield improvements in runtime. However, they should be used with caution as they will yield non-optimal results if the assumptions implicit in each option are violated.",
)
special_cases.add_argument(
    "--use_beta_se",
    default=False,
    action="store_true",
    help="If turned on, MTAG will use the provided BETA and SE columns to perform estimation. By default, MTAG uses the Z-statistic.",
)
special_cases.add_argument(
    "--no_overlap",
    default=False,
    action="store_true",
    help="Imposes the assumption that there is no sample overlap between the input GWAS summary statistics. MTAG is performed with the off-diagonal terms on the residual covariance matrix set to 0.",
)
special_cases.add_argument(
    "--perfect_gencov",
    default=False,
    action="store_true",
    help="Imposes the assumption that all traits used are perfectly genetically correlated with each other. The off-diagonal terms of the genetic covariance matrix are set to the square root of the product of the heritabilities",
)
special_cases.add_argument(
    "--equal_h2",
    default=False,
    action="store_true",
    help="Imposes the assumption that all traits passed to MTAG have equal heritability. The diagonal terms of the genetic covariance matrix are set equal to each other. Can only be used in conjunction with --perfect_gencov",
)
special_cases.add_argument(
    "--force",
    default=False,
    action="store_true",
    help="Force MTAG estimation even though the mean chi2 is small.",
)

fdr_opts = parser.add_argument_group(
    title="Max FDR calculation",
    description="These options are used for the calculation of an upper bound on the false disovery under the model described in Supplementary Note 1.1.4 of Turley et al. (2017). Note that there is one of three ways to define the space of grid points over which the upper bound is searched. ",
)
fdr_opts.add_argument(
    "--fdr", default=False, action="store_true", help="Perform max FDR calculations"
)
fdr_opts.add_argument(
    "--skip_mtag",
    default=False,
    action="store_true",
    help="Skip calculations of MTAG and perform FDR only.",
)
fdr_opts.add_argument(
    "--grid_file",
    default=None,
    action="store",
    help="Pre-set list of grid points. Users can define a list of grid points over which the search is conducted. The list of grid points should be passed in text file as a white-space delimited matrix of dimnesions, G x S, where G is the number of grid points and S = 2^T is the number of possible causal states for SNPs. States are ordered according to a tree-like recursive structure from right to left. For example, for 3 traits, with the triple TFT denoting the state for which SNPs are causal for State 1, not causal for state 2, and causal for state 3, then the column ordering of probabilities should be: \nFFF FFT FTF FTT TFF TFT TTF TTT\n There should be no headers, or row names in the file. Any rows for which (i) the probabilities do not sum to 1, the prior of a SNP being is causal is 0 for any of the traits, and (iii) the resulting genetic correlation matrix is non positive definite will excluded in the search.",
)
fdr_opts.add_argument(
    "--fit_ss",
    default=False,
    action="store_true",
    help="This estimates the prior probability that a SNP is null for each trait and then proceeds to restrict the grid search to the set of probability vectors that sum to the prior null for each trait. This is useful for restrict the search space of larger-dimensional traits.",
)
fdr_opts.add_argument(
    "--intervals",
    default=10,
    action="store",
    type=int,
    help="Number of intervals that you would like to partition the [0,1] interval. For example example, with two traits and --intervals set 10, then maxFDR will calculated over the set of feasible points in {0., 0.1, 0.2,..,0.9,1.0}^2.",
)
fdr_opts.add_argument(
    "--cores",
    default=1,
    action="store",
    type=int,
    help="Number of threads/cores use to compute the FDR grid points for each trait.",
)
fdr_opts.add_argument(
    "--p_sig",
    default=5.0e-8,
    type=float,
    action="store",
    help="P-value threshold used for statistical signifiance. Default is p=5.0e-8 (genome-wide significance).",
)
fdr_opts.add_argument(
    "--n_approx",
    default=True,
    action="store_true",
    help="Speed up FDR calculation by replacing the sample size of a SNP for each trait by the mean across SNPs (for each trait). Recommended.",
)
...
misc.add_argument(
    ...
    metavar="FOLDER_PATH",
    type=str,
    help="Specify folder of the ld reference panel (split by chromosome) that will be used in the estimation of the error VCV (sigma). This option is passed to --ref-ld-chr and --w-ld-chr when running LD score regression. The default is to use the reference panel of LD scores computed from 1000 Genomes European subjects (eur_w_ld_chr) that is included with the distribution of MTAG",
)
misc.add_argument(
    "--time_limit",
    default=100.0,
    type=float,
    action="store",
    help="Set time limit (hours) on the numerical estimation of the variance covariance matrix for MTAG, after which the optimization routine will complete its current iteration and perform MTAG using the last iteration of the genetic VCV.",
)
...
misc.add_argument(
    ...
    default=False,
    action="store_true",
    help="Results files will have standardized effect sizes, i.e., the weights 1/sqrt(2*MAF*(1-MAF)) are not applied when outputting MTAG results, where MAF is the minor allele frequency.",
)
misc.add_argument(
    "--tol",
    default=1e-6,
    type=float,
    help="Set the relative (x) tolerance when numerically estimating the genetic variance-covariance matrix. Not recommended to change unless you are facing strong runtime constraints for a large number of traits.",
)
misc.add_argument(
    "--numerical_omega",
    default=False,
    action="store_true",
    help="Option to use the MLE estimator of the genetic VCV matrix, implemented through a numerical routine.",
)
misc.add_argument(
    "--verbose",
    default=False,
    action="store_true",
    help="When used, will include output from running ldsc scripts as well additional information (such as optimization routine information.",
)
misc.add_argument(
    "--chunksize", default=1e7, type=int, help="Chunksize for reading in data."
)
misc.add_argument(
    "--stream_stdout",
    default=False,
    action="store_true",
    help="Will streat mtag processing on console in addition to writing to log file.",
)
misc.add_argument(
    "--median_z_cutoff",
    default=DEFAULT_MEDIAN_Z_THRESHOLD,
    type=float,
    help="Maximum allowed median Z-score for sumstats during input QC",
)
...
if __name__ == "__main__":
    ...
    if args.use_beta_se:
        raise RuntimeError(
            "Due to bugs in the beta-se code, this option has been temporarily removed from the MTAG software. Dec 2, 2021"
        )
    ...
    if args.skip_mtag:
        ...
        for t in range(T):
            df_d[t] = pd.read_csv(
                "{}_trait_{}.txt".format(args.out, t + 1),
                index_col=None,
                delim_whitespace=True,
            )
            if t == 0:
